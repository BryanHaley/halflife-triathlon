...
bool CHudStrafeGuide::VidInit()
{
	return true;
}

bool CHudStrafeGuide::Draw(float time)
{
	...
	for (int i = 0; i < 4; ++i)
	{
		...
		for (int iCopy = -8; iCopy <= 8; ++iCopy)
		{
			...
			FillRGBA(boxLeftI, height, boxRightI - boxLeftI, size, color, 60);
		}
	}
...
void CHudStrafeGuide::Update(struct ref_params_s* pparams)
{
	...
	bool onground = (bool) pparams->onground;
	...
	if (onground)
		inputAbs = std::min(inputAbs, (double) pparams->movevars->maxspeed);
	else
		inputAbs = std::min(inputAbs, (double) 30);
